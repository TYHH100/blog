...
# 游戏依赖（可选，指定此游戏依赖的其他游戏）
# 格式: GAME_DEPENDENCIES["游戏显示名称"]="依赖游戏名称"
# 注意: 依赖游戏的名称必须已经在 GAME_APPS 中定义
# 
# 支持多个依赖，用空格分隔:
# - 单个依赖: GAME_DEPENDENCIES["游戏名"]="Team Fortress 2"
# - 多个依赖: GAME_DEPENDENCIES["游戏名"]="Team Fortress 2 Counter-Strike 2"
# 
# 注意事项:
# - 如果游戏名称包含空格（如 "Team Fortress 2"），系统会正确处理
# - 依赖游戏会按顺序安装/更新
# - 如果某个依赖游戏未找到，会跳过并继续处理其他依赖
# - 示例: 如果这个游戏需要 Team Fortress 2 服务器作为依赖
GAME_DEPENDENCIES["自定义游戏示例[切勿安装]"]=""
...
install_dependencies_examples() {
    ...
    case "$OS_INFO" in
        ...
        *)
            # 其他未定义的系统
            msg_warn "当前系统 ($OS_INFO) 未定义额外依赖，尝试跳过..."
